...
var __swift = module.exports = exports = function (pathToAppConfig_)
{
    ...
    config.swift                    = $typeUtil.isObject(config.swift) ? config.swift : {};
    config.swift.use                = $typeUtil.isObject(config.swift.use) ? config.swift.use : {};
    config.swift.use.dbManager      = typeof config.swift.use.dbManager === 'boolean' ? config.swift.use.dbManager : false;
    config.swift.use.helperManager  = typeof config.swift.use.helperManager === 'boolean' ? config.swift.use.helperManager : false;
    ...
    if (config.swift.use.dbManager && $typeUtil.isObject(config.db))
    {
        var adapterNames = Object.keys(config.db).map(function (adapterName) { return adapterName; }),
            adapters = dbManager.useAdapters(adapterNames).getAdapters();

        Object.keys(adapters).forEach(function (adapterName)
        {
            var adapter = adapters[adapterName],
                connectionsParams = config.db[adapterName];
 ...
 * @type {DbManager}
 */
__swift.dbManager = null;

/**
 * @type {helperManager}
 */
__swift.helperManager = null;