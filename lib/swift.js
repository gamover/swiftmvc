...
var __swift = module.exports = exports = function (pathToAppConfig)
{
    ...
    if (pathToAppConfig)
    {
        configurator.setPathToAppConfig(pathToAppConfig);
    }
    ...
    var pathToRoutes = _path.dirname(configurator.getPathToConfig()) + '/routes.json';

    if (_path.existsSync(pathToRoutes) && _fs.statSync(pathToRoutes).isFile() && _path.extname(pathToRoutes) === '.json')
    {
        var routes_ = JSON.parse(_fs.readFileSync(pathToRoutes, 'UTF-8'));

        url.setRoutes(routes_);
