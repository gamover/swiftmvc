...
function UsedSystem (key, prepare) {
    this.key = key;
    if (typeof prepare === 'function') this.prepare = prepare;
    else this.prepare = function () {};
}
...
function Swift ()
{
    ...
    this.SYSTEMS.DB_MANAGER = new UsedSystem('DB_MANAGER', function ()
    {
        if (self.dbManager === null) self.dbManager = new DbManager();
    });

    this.SYSTEMS.HELPER_MANAGER = new UsedSystem('DB_MANAGER', function ()
    {
        if (self.helperManager === null) self.helperManager = new HelperManager();
    });
}
...
Swift.prototype.createApp = function (pathToAppConfig_)
{
    ...
    if (self._usedSystems[self.SYSTEMS.DB_MANAGER.key])
    {
    ...
    if (self._usedSystems[self.SYSTEMS.HELPER_MANAGER.key])
    {
...
Swift.prototype.use = function (list)
{
    ...
    {
        if (!system instanceof UsedSystem) return;
        if (typeof self._usedSystems[system.key] !== 'undefined') return;

        system.prepare();

        self._usedSystems[system.key] = true;
    });
